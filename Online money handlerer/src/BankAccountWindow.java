...
public class BankAccountWindow extends JFrame {
	...
	public BankAccountWindow(String username) {
		...
		JLabel creditLabel = new JLabel("Credits: " + bank.getCredits(username));
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				boolean ifTrue = bank.removeCredit(username, Double.parseDouble(amountToSend.getText()));
				
				if (ifTrue) {
					
					boolean ifElseTrue = bank.addCredit(userToSend.getText(), Double.parseDouble(amountToSend.getText()));
					
					if (ifElseTrue) {
						JOptionPane.showMessageDialog(null, "Successfully sended");
						creditLabel.setText("Credits: " + bank.getCredits(username));
					} else {
					...
					JOptionPane.showMessageDialog(null, "Not enough credit to send");
				}
